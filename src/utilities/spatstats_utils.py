...
class ComputeStatistics():
    ...
    def compute_Fest(self, ppp_r, r_des = None, correction = 'rs', var_stb=False):
        """Compute the functional statistic F, also referred as empty space function, 
        for a point-process ```ppp_r```.
...
def compute_hatS_statistic(radius, rmax, Sm, S0, pnorm=2, rmin=0.0, one_sided=False):
...
def compute_statistics(sts, cs, simulation_pos, pos_exp, radius, rmax, pnorm, one_sided):
    ...
    tm = compute_hatS_statistic(radius, rmax, Sm, S0, pnorm=pnorm,one_sided=one_sided)
    # sort values of t for null hypothesis
    tm = np.sort(tm, axis=0)[::-1, :]
    # Compute empirical statistic texp
    t_exp = compute_hatS_statistic(radius, rmax, Sexp, S0, pnorm=pnorm,one_sided=one_sided)
...
def compute_monte_carlo_sims(signal,
    ...
    pos_emp = find_zeros_of_spectrogram(stf)
    ...
    if signal.dtype == complex128:
        ...
        valid_zeros = np.zeros((pos_emp.shape[0],),dtype=bool)
        valid_zeros[(T<pos_emp[:,0])]=True 
        pos_emp = pos_emp[valid_zeros]

    pos_emp /= T
    u_r, v_r, b_u, b_v = compute_positions_and_bounds(pos_emp)
    pos_emp = spatstat.geom.ppp(u_r, v_r, b_u, b_v)

    # Load precomputed PP's.
    if use_precomputed:
        try:
            with open(os.path.join('.','ppp_simulations_N_{}.mcsim'.format(N)), 'rb') as handle:
                aux = pickle.load(handle)
                ppp_sim = aux['list_ppp']
        except:
            print('Simulations not found. Running and saving MC Simulation ...')
            ppp_sim_R, ppp_sim = generate_white_noise_zeros_pp(N,2499,complex_noise=complex_signal)
            aux = {'list_ppp_R':ppp_sim_R,
                   'list_ppp':ppp_sim,
                        'N' : N,
                        'nsim' : 2499,
                        }
            with open('ppp_simulations_N_{}.mcsim'.format(N), 'wb') as handle:
                pickle.dump(aux, handle, protocol=pickle.HIGHEST_PROTOCOL)  
        
        ppp_sim = [ppp_sim[i] for i in np.random.choice(len(ppp_sim), MC_reps, replace=False)]

    else:
        ppp_sim = list()
        _, ppp_sim = generate_white_noise_zeros_pp(N,MC_reps,complex_noise=complex_signal)
...
def compute_envelope_test(signal, 
    ...
    Args:
        ...
        determined by the zeros of spectrogram of the signal. Defaults to 'F'.
...
def generate_white_noise_zeros_pp(N, nsim, Nfft=None,complex_noise=False, parallel=False):
    ...
    list_ppp_R = rbase.list() # R list
    list_ppp = [] # Python list.
    for pos in list_of_zeros:
        pos = np.array(pos)/T
        u_r, v_r, b_u, b_v = compute_positions_and_bounds(pos)
        ppp_noise = spatstat.geom.ppp(u_r, v_r, b_u, b_v)
        # ppp_noise = spatstat.geom.ppp(u_r, v_r)        
        list_ppp.append(copy.deepcopy(ppp_noise))
        list_ppp_R = rbase.c(list_ppp_R, spatstat.geom.as_solist(ppp_noise))

    list_ppp_R = spatstat.geom.as_solist(list_ppp_R)
    return list_ppp_R, list_ppp
...
def compute_rank_envelope_test(signal,
    ...
    if use_precomputed: # For fast reproduction, use precomputed PPs.
        try:
            with open(os.path.join('.','ppp_simulations_N_{}.mcsim'.format(N,nsim)), 'rb') as handle:
                aux = pickle.load(handle)
                ppp_sim = aux['list_ppp_R'] # Load a precomputed R list of PPs.

        except: # If there are no precomputed PPs, compute and save them
            print('Simulations not found. Running and saving MC Simulation ...')
            ppp_sim_R, ppp_sim = generate_white_noise_zeros_pp(N,nsim,complex_noise=complex_signal)
            aux = {'list_ppp_R':ppp_sim_R,
                   'list_ppp':ppp_sim,
                        'N':N,
                        'nsim':nsim
            ...
            with open('ppp_simulations_N_{}.mcsim'.format(N,nsim), 'wb') as handle:
                pickle.dump(aux, handle, protocol=pickle.HIGHEST_PROTOCOL)   
            
            ppp_sim=ppp_sim_R
    else:
        if ppp_sim is None:
            ppp_sim,_ = generate_white_noise_zeros_pp(N,nsim,complex_noise=complex_signal)
